<template>
  <div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      ...
      <div class="bg-white rounded-xl border p-4">
        <p class="text-xs text-gray-400 uppercase">Ports Up</p>
        <p class="text-2xl font-bold mt-1 text-emerald-600">{{ portsUp }} / 10</p>
      </div>
      <div class="bg-white rounded-xl border p-4">
        <p class="text-xs text-gray-400 uppercase">SSE</p>
    ...
    <div class="bg-white rounded-xl border p-6">
      <h2 class="text-sm font-semibold text-gray-400 uppercase mb-4">Port Status</h2>
      <div class="flex gap-2 flex-wrap">
        <div v-for="port in ports" :key="port.port"
          ...
          <div class="text-[10px] mt-1" :class="port.link !== 'Link Down' ? 'text-emerald-600' : 'text-gray-300'">
            {{ port.link === 'Link Down' ? 'Down' : port.link.replace('MbpsFull','M').replace('MbpsHalf','M/H') }}
          </div>
