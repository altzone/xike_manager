  ...
  <div class="space-y-6">
    ...
    <div v-if="loaded">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
          <h3 class="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">SNTP</h3>
          <ToggleRow label="SNTP Enabled" :value="sntp.enabled" @toggle="sntp.enabled = !sntp.enabled; applySntp()"/>
          <div v-if="sntp.enabled" class="mt-3 space-y-2">
            <div><label class="text-xs text-gray-500">Server</label><input v-model="sntp.server" @blur="applySntp" class="inp w-full" placeholder="pool.ntp.org"/></div>
            <div><label class="text-xs text-gray-500">Poll (30-99999s)</label><input v-model.number="sntp.poll" type="number" min="30" max="99999" @blur="applySntp" class="inp w-full"/></div>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
          <ToggleRow label="STP" :value="stp.enabled" @toggle="stp.enabled = !stp.enabled; applyStp()"/>
          <p class="text-xs text-gray-400 mt-1">Spanning Tree Protocol prevents network loops</p>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
          <ToggleRow label="Storm Control" :value="storm.enabled" @toggle="storm.enabled = !storm.enabled; applyStorm()"/>
          <p class="text-xs text-gray-400 mt-1">Limits broadcast/multicast storm traffic</p>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
          <ToggleRow label="IGMP Snooping" :value="igmp.enabled" @toggle="igmp.enabled = !igmp.enabled; applyIgmp()"/>
          <p class="text-xs text-gray-400 mt-1">Optimizes multicast traffic delivery</p>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
          <ToggleRow label="Energy Efficient Ethernet" :value="eee.enabled" @toggle="eee.enabled = !eee.enabled; applyEee()"/>
          <p class="text-xs text-gray-400 mt-1">Reduces power consumption during low traffic periods</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
          <h3 class="text-sm font-semibold text-gray-700 mb-3">Port Mirror</h3>
          <div class="space-y-2">
            <div class="flex items-center gap-2">
              <label class="text-xs text-gray-500 w-24">Monitor port:</label>
              <select v-model.number="mirror.monitoring_port" class="inp flex-1">
            ...
            <div v-if="mirror.monitoring_port > 0" class="space-y-2">
              <div class="flex items-center gap-2">
                <label class="text-xs text-gray-500 w-24">Direction:</label>
                <select v-model="mirror.ingress" class="inp"><option value="1">Ingress</option><option value="0">Off</option></select>
                <select v-model="mirror.egress" class="inp"><option value="1">Egress</option><option value="0">Off</option></select>
              </div>
              <div>
              ...
              <button @click="applyMirror" class="px-3 py-1.5 bg-indigo-600 text-white text-xs rounded-lg hover:bg-indigo-700 transition">Apply Mirror</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5 mb-6">
        ...
        <div class="flex items-center gap-2 mb-3">
          ...
          <button @click="addStaticMac" class="px-3 py-1.5 bg-indigo-600 text-white text-xs rounded-lg hover:bg-indigo-700 transition">Add</button>
        </div>
        <div v-if="staticMacs.length" class="text-sm divide-y divide-gray-50">
          <div v-for="(m, i) in staticMacs" :key="i" class="flex items-center justify-between py-1.5">
            <span class="font-mono text-gray-700">{{ m.mac }}</span>
            <span class="text-gray-400 text-xs">Port {{ m.port }} / FID {{ m.fid }}</span>
          </div>
        </div>
      ...
      <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5 mb-6">
        <div class="flex items-center justify-between mb-3">
          ...
          <div class="flex gap-2">
            <label class="px-3 py-1.5 bg-gray-100 text-gray-600 text-xs rounded-lg hover:bg-gray-200 transition cursor-pointer">
              Import <input type="file" accept=".json" @change="importFile" class="hidden"/>
            </label>
            <button @click="showSaveSnapshot = true" class="px-3 py-1.5 bg-indigo-600 text-white text-xs rounded-lg hover:bg-indigo-700 transition">Save Snapshot</button>
          </div>
        </div>
        <div v-if="snapshots.length" class="divide-y divide-gray-50">
          <div v-for="s in snapshots" :key="s.id" class="flex items-center justify-between py-2.5 group">
            <div>
            ...
            <div class="flex gap-1.5 opacity-0 group-hover:opacity-100 transition">
              <button @click="viewSnapshot(s)" class="px-2 py-1 text-xs bg-gray-100 text-gray-600 rounded hover:bg-gray-200">View</button>
        ...
        <p v-else class="text-xs text-gray-400 py-2">No snapshots yet</p>
      </div>

      <!-- Danger Zone -->
      <div class="bg-white rounded-xl border border-red-200 shadow-sm p-5">
        <h3 class="font-semibold text-red-600 mb-2">Danger Zone</h3>
        <button @click="doReboot" class="px-4 py-2 bg-red-50 text-red-600 text-sm rounded-lg hover:bg-red-100 border border-red-200 transition">Reboot Switch</button>
      </div>
    </div>
    ...
    <div v-if="viewing" class="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50" @click.self="viewing = null">
      <div class="bg-white rounded-2xl p-6 w-full max-w-2xl shadow-2xl max-h-[80vh] overflow-auto">
        <div class="flex justify-between mb-4"><h2 class="text-lg font-bold">{{ viewing.name }}</h2>
          <button @click="viewing = null" class="text-gray-400 hover:text-gray-600"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
        </div>
...
async function load() {
  try {
    ...
    timeData.value = t
    sntp.enabled = t.sntp_state === '1'; sntp.server = t.sntp_server_ip || 'pool.ntp.org'; sntp.poll = parseInt(t.sntp_poll) || 64
...
async function applyStorm() { await api(`/api/switches/${props.switchId}/storm`, { method: 'POST', body: JSON.stringify({ enabled: storm.enabled, rate: storm.rate }) }); flash('Storm control updated') }
...
async function addStaticMac() { await api(`/api/switches/${props.switchId}/mac/static/add`, { method: 'POST', body: JSON.stringify(staticMac) }); flash('Static MAC added'); staticMac.mac = ''; await load() }
...
async function saveSnapshot() { await api(`/api/switches/${props.switchId}/snapshots`, { method: 'POST', body: JSON.stringify({ name: snapshotName.value }) }); showSaveSnapshot.value = false; snapshotName.value = ''; flash('Snapshot saved'); await load() }
async function viewSnapshot(s) { viewing.value = await api(`/api/switches/${props.switchId}/snapshots/${s.id}`) }
async function downloadSnapshot(s) { const full = await api(`/api/switches/${props.switchId}/snapshots/${s.id}`); const blob = new Blob([JSON.stringify(full, null, 2)], { type: 'application/json' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `${s.name.replace(/\s+/g, '_')}.json`; a.click() }
...
async function importFile(e) { const file = e.target.files[0]; if (!file) return; try { const data = JSON.parse(await file.text()); await api(`/api/switches/${props.switchId}/snapshots/import`, { method: 'POST', body: JSON.stringify({ name: `Import: ${file.name}`, config: data.config || data }) }); flash('Imported'); await load() } catch(err) { flash('Invalid JSON', false) }; e.target.value = '' }
...
const ToggleRow = {
  props: ['label', 'value'],
  emits: ['toggle'],
  template: `<div class="flex items-center justify-between"><span class="text-sm font-semibold text-gray-700">{{ label }}</span><button @click="$emit('toggle')" class="relative w-11 h-6 rounded-full transition-colors duration-200" :class="value ? 'bg-emerald-500' : 'bg-gray-300'"><span class="absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform duration-200" :class="value ? 'translate-x-5' : ''"></span></button></div>`
}
